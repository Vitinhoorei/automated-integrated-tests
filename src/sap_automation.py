...
class SapAutomation:
    ...
    def _capture_object_region(self, obj_id: str, out_path: str, pad: int = 12) -> bool:
        try:
            ...
            left, top, width, height = int(getattr(obj, "ScreenLeft", 0)), int(getattr(obj, "ScreenTop", 0)), int(getattr(obj, "Width", 0)), int(getattr(obj, "Height", 0))
    ...
    def _capture_field_crop_from_hardcopy(self, field_id: str, out_path: str, pad: int = 12) -> str:
        if not field_id or not out_path: return ""
    ...
    def _capture_error_evidence(self, out_path: str, source: str, field_id: str = "") -> str:
        ...
        src = (source or "").upper()
        if src == "POPUP":
            if self._capture_object_region("wnd[1]", out_path, pad=20): return out_path
        if src == "STATUSBAR":
            if self._capture_object_region("wnd[0]/sbar", out_path, pad=8): return out_path
            ...
            for key, (obj, obj_type, value) in campos_normais.items():
                ...
                try:
                    if obj_type == "GuiComboBox":
                        ...
                        try: obj.text = val_str
                        except Exception: obj.key = val_str
                    action_taken = "TEXT"
                except Exception:
                    remaining_params[key] = value
            
            for k, v in abas.items(): remaining_params[k] = v[1]
            for k, v in botoes.items(): remaining_params[k] = v[1]
        ...
        try:
                ...
                if params_to_fill:
                    if not popups_dismissed:
                        if action_taken == "TEXT":
                            ...
                            if str(self.session.Info.Program).strip() == "SAPLIQS0" and tcode.upper() == "IW31":
            ...
            for tentativa in range(3): 
                try:
                    ...
                    if not clicou: self.session.findById("wnd[0]").sendVKey(11)
                    time.sleep(1.5)
                    while self._popup_exists():
                        txt = self._popup_text()
                        if txt: popup_msgs.append(txt)
